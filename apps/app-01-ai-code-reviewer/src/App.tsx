...
const sampleReview: ReviewIssue[] = [
  {
    ...
    line: 4,
    severity: 'warning',
    message: 'Recursive function may cause stack overflow for large n',
    suggestion: 'Consider using iterative approach or memoization',
  ...
  },
  {
    ...
    line: 2,
    severity: 'info',
    message: 'Base case check can be simplified',
    suggestion: 'Use === 0 comparison for clarity',
    category: 'style',
  },
  {
    id: '3',
    ...
    suggestion: 'Add type checking for negative numbers',
    category: 'bug',
  },
]
    ...
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an expert code reviewer. Analyze code and respond with ONLY valid JSON containing issues found.',
            },
            {
              role: 'user',
              content: `Analyze this code and provide issues in JSON format:\n\n${code}`,
            },
          ],
          temperature: 0.3,
          max_tokens: 2000,
      ...
      if (jsonMatch) {
        ...
        const newStats: ReviewStats = {
          ...
          errors: (parsed.issues || []).filter((i: any) => i.severity === 'error').length,
          warnings: (parsed.issues || []).filter((i: any) => i.severity === 'warning').length,
          suggestions: (parsed.issues || []).filter((i: any) => i.severity === 'info').length,
        }
        setStats(newStats)
        setReviewResults(parsed.issues || [])
      } else {
        // Fallback if no valid JSON
        setStats({ score: 85, issues: 0, errors: 0, warnings: 0, suggestions: 0 })
        setReviewResults([])
      }
    } catch (error) {
              ...
              <div className="p-0">
                <div className="flex">
                  <div className="w-12 bg-[#161b22] border-r border-[#30363d] py-4 text-right pr-3">
                    {code.split('\n').map((_, i) => (
                      <div key={i} className="text-xs text-gray-500 leading-6 font-mono">
                        {i + 1}
                      </div>
                    ))}
                  </div>
                  <textarea
                    ...
                    className="flex-1 bg-[#0d1117] p-4 font-mono text-sm text-[#c9d1d9] resize-none focus:outline-none leading-6"
                    style={{ minHeight: '400px' }}
                  />
                </div>
              </div>
              <div className="px-4 py-3 border-t border-[#30363d]">
                <Button onClick={runReview} loading={isReviewing} className="w-full">
                  ...
                  {isReviewing ? (
                    ...
                    reviewResults.map((issue) => (
                      <div
                        ...
                        className={`p-4 rounded-xl border cursor-pointer transition-all ${getSeverityColor(issue.severity)} ${selectedIssue === issue.id ? 'ring-2 ring-violet-500/50' : ''}`}
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-xs px-2 py-0.5 bg-[#30363d]/50 rounded capitalize">
                                {issue.category}
                              </span>
                            </div>
                            <p className="text-sm font-medium">{issue.message}</p>
                            {selectedIssue === issue.id && (
                              <div className="mt-3 p-3 bg-[#161b22]/80 rounded-lg">
                    ...
                    <div className="flex items-center justify-center py-12 text-gray-400">
                      <div className="text-center">
                        ...
                        <p className="text-sm">Paste code and click "Run AI Review"</p>
                      </div>
                    </div>
                  )}
                </div>
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-white overflow-hidden">
      <div className="fixed inset-0 z-0">
        ...
        <div
          className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-indigo-500/5 rounded-full blur-3xl animate-pulse"
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed">
              Get instant, intelligent code reviews. Catch bugs, improve performance, and follow
              best practices.
            </p>
            ...
            <div className="bg-[#0d1117]/80 backdrop-blur-xl border border-[#30363d] rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="p-6 grid lg:grid-cols-2 gap-6">
                    ...
                    <div className="flex">
                      ...
                      <div className="flex-1">
                        <div className="leading-7">
                          ...
                          <span className="text-[#79c0ff]">fibonacci</span>(n) {'{'}
                        </div>
                        <div className="leading-7 pl-4">
                          <span className="text-[#ff7b72]">if</span> (n {'<='} 1){' '}
                          <span className="text-[#ff7b72]">return</span> n;
                        </div>
                        <div className="leading-7 pl-4">
                          <span className="text-[#ff7b72]">return</span> fibonacci(n - 1) +
                          fibonacci(n - 2);
                        </div>
                ...
                <div className="space-y-3">
                  <div className="text-xs text-violet-400 mb-2 uppercase tracking-wider flex items-center gap-2">
            ...
            {[
              {
                ...
                desc: 'Detect vulnerabilities and security risks',
                color: 'text-green-400',
              },
              {
                icon: Zap,
                title: 'Performance Tips',
                desc: 'Optimize for better speed and efficiency',
                color: 'text-amber-400',
              },
              {
                icon: CheckCircle,
                title: 'Best Practices',
                desc: 'Follow coding standards and best practices',
                color: 'text-blue-400',
              },
            ].map((feature, i) => (
              <div
                key={i}
                className="p-6 bg-[#0d1117] border border-[#30363d] rounded-2xl hover:border-violet-500/50 transition-all"
              >
